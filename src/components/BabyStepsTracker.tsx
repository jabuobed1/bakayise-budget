...
export const BabyStepsTracker: React.FC<BabyStepsTrackerProps> = ({
  ...
  // Main income: primary_salary, spouse_salary or titles with 'salary', excluding side/bonus/freelance/rental/other
  const mainIncomes = (incomes || []).filter((inc) => {
    const t = inc.type;
    const isMainSalary = t === 'primary_salary' || t === 'spouse_salary';
    const isOtherExcluded = t === 'freelance' || t === 'side_hustle' || t === 'bonus' || t === 'rental' || t === 'other';
    const titleHasSalary = (inc.title || '').toLowerCase().includes('salary');
    return isMainSalary || (titleHasSalary && !isOtherExcluded);
  });
  const totalMainIncome = mainIncomes.reduce((sum, inc) => sum + (inc.amount || 0), 0);
  const retirement15PercentTarget = totalMainIncome * 0.15;

  // Retirement budget entries in current active cycle
  const retirementCategories = categories.filter((c) => {
    const isTag = c.tag === 'retirement' || (c as any).tags?.includes('retirement');
    const nameLower = (c.name || '').toLowerCase();
    const isName =
      nameLower.includes('retirement') ||
      nameLower.includes('ra ') ||
      nameLower.endsWith('ra') ||
      nameLower.includes('pension') ||
      nameLower.includes('provident');
    return isTag || isName;
  });
  const totalRetirementBudgeted = retirementCategories.reduce((sum, c) => sum + (c.allocatedAmount || 0), 0);
  const retirementProgress =
    retirement15PercentTarget > 0
      ? Math.min(100, (totalRetirementBudgeted / retirement15PercentTarget) * 100)
      : 0;
  const isRetirementOnTrack =
    retirement15PercentTarget > 0 && totalRetirementBudgeted >= retirement15PercentTarget;

  // Explicitly assigned retirement / investment accounts
  ...
  const bondAccounts = accounts.filter(
    (a) => a.type === 'home_loan' || a.babyStepAssignment === 6 || (a as any).babyStepAssignments?.includes(6)
  );
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {isRetirementOnTrack && (
                    <span className="text-[10px] font-semibold px-2 py-0.2 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 shrink-0">
                      15% Met ✓
                    </span>
                  )}
                </div>
                ...
                <div className="mt-1 space-y-1 text-xs text-slate-400">
                  <p>
                    15% Target (from {formatZARCompact(totalMainIncome)} salary):{' '}
                    <strong className="text-purple-300 font-mono">{formatZAR(retirement15PercentTarget)}/mo</strong>
                  </p>
                  <p>
                    Allocated in Current Cycle:{' '}
                    <strong className="text-white font-mono">{formatZAR(totalRetirementBudgeted)}/mo</strong>
                    {retirement15PercentTarget > totalRetirementBudgeted && (
                      <span className="text-amber-400 ml-1.5 font-medium">
                        ({formatZAR(retirement15PercentTarget - totalRetirementBudgeted)} to go)
                      </span>
                    )}
                  </p>
                </div>
                ...
                <div className="mt-2.5">
                  <div className="flex items-center justify-between text-[11px] text-slate-400 mb-1">
                    <span>15% Goal Progress</span>
                    <span className="font-mono text-purple-300 font-bold">{retirementProgress.toFixed(0)}%</span>
                  </div>
                  <div className="w-full bg-[#2C2C2E] rounded-full h-2 overflow-hidden p-0.5 border border-white/[0.06]">
                    <div
                ...
                {step4AssignedAccounts.length > 0 && (
                  <div className="mt-2.5 flex flex-wrap gap-1.5">
                    {step4AssignedAccounts.map((a) => (
                      <span
                        key={a.id}
                        className="text-[10px] px-2 py-0.5 rounded bg-[#2C2C2E] text-slate-300 border border-white/5 font-mono"
                      >
